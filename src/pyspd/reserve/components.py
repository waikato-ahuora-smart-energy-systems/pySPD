...
class ReserveDomainsComponent(ModelComponent):
    ...
    def build(self, context: BuildContext) -> Mapping[str, Any]:
        ...
        block.RiskClass = pyo.Set(ordered=True, initialize=RISK_CLASSES)
        ...
        block.EnergyBreakpoint = pyo.Set(
            ordered=True, initialize=ENERGY_BREAKPOINTS
        )
        block.ReserveBreakpoint = pyo.Set(
            ordered=True, initialize=RESERVE_BREAKPOINTS
        )
        ...
        block.Shortfall = pyo.Set(
            ...
            initialize=sorted(
                ...
                for island in data.islands
                for reserve_class in RESERVE_CLASSES
                for risk in HVDC_RISKS | MANUAL_RISKS | HVDC_SECONDARY_RISKS
                if risk in CE_RISKS
            ),
        )
        block.ShortfallUnit = pyo.Set(
            ...
            initialize=sorted(
                ...
                for generator in data.risk_generators
                for reserve_class in RESERVE_CLASSES
                for risk in GEN_RISKS | HVDC_SECONDARY_RISKS
                if risk in CE_RISKS
            ),
        )
        block.ShortfallGroup = pyo.Set(
            ...
            initialize=sorted(
                (group[0], group[1], group[2], group[3], reserve_class, group[4])
                for group in data.island_risk_group
                if group[4] in CE_RISKS & GEN_RISKS
                for reserve_class in RESERVE_CLASSES
            ),
        )
...
class ReserveOfferComponent(ModelComponent):
    ...
    supported_formulations = _SUPPORTED
    requires = frozenset(
        {"reserve_data", "reserve_domains", "generation", "purchase"}
    )
    provides = frozenset(
        {
            "reserve",
    ...
    def build(self, context: BuildContext) -> Mapping[str, Any]:
        ...
        block.EnergyAndReserveMaximum = pyo.Constraint(
            [(offer, reserve_class) for offer in case.offers for reserve_class in RESERVE_CLASSES],
            rule=lambda _b, ca, dt, offer, reserve_class: (
                generation[ca, dt, offer]
                + data.reserve_maximum_factor.get(
                    (ca, dt, offer, reserve_class), 0.0
                )
                * sum(
                    _b.Reserve[ca, dt, offer, reserve_class, reserve_type]
    ...
    def build(self, context: BuildContext) -> Mapping[str, Any]:
        ...
        for key in domains.ShortfallUnit:
            block.ShortfallDefinitions.add(
                ...
                == sum(
                    block.ReserveShortfallUnitBlock[*key, tranche]
                    for tranche in BLOCKS
                )
            )
...
class ReserveRiskComponent(ModelComponent):
    ...
    def build(self, context: BuildContext) -> Mapping[str, Any]:
        ...
        block.HVDCManualIslandRisk = pyo.Var(
            secondary_manual_domain, domain=pyo.Reals
        )
        block.HVDCReceived = pyo.Var(domains.Island, domain=pyo.Reals)
        block.RiskOffset = pyo.Var(
            [key for key in domains.IslandRisk if key[4] in HVDC_RISKS],
            domain=pyo.Reals,
        )
        ...
        for key in domains.IslandRisk:
            ...
            shared = _optional(effective, key)
            if risk in HVDC_RISKS:
                block.Constraints.add(
                    block.RiskOffset[key]
                    == data.free_reserve.get(key, 0.0)
                    + (data.hvdc_pole_ramp_up.get(key, 0.0) if risk in CE_RISKS else 0.0)
                )
                block.Constraints.add(
        ...
        for key in domains.GeneratorRisk:
            ...
            block.Constraints.add(block.IslandRisk[ca, dt, island, reserve_class, risk] >= block.GeneratorIslandRisk[key])
        for key in domains.RiskGroup:
            ...
            gross = (
                sum(
                    branch_flow[ca, dt, branch]
                    * factor
                    for (f_ca, f_dt, f_group, branch, f_risk), factor in data.directional_risk_factor.items()
            ...
            block.Constraints.add(block.IslandRisk[ca, dt, island, reserve_class, risk] >= block.GroupIslandRisk[key])
        for key in secondary_generator_domain:
            ...
            gross = (
                ...
                + sum(
                    ...
                    + sum(
                        reserve[
                            ca, dt, secondary, reserve_class, reserve_type
                        ]
                        for reserve_type in RESERVE_TYPES
                    )
        ...
        for key in secondary_manual_domain:
            ...
            block.Constraints.add(
                ...
                * (
                    ...
                    - data.free_reserve.get(key, 0.0)
                    + block.HVDCReceived[ca, dt, island]
                    - data.hvdc_secondary_subtractor.get(
                        (ca, dt, island), 0.0
                    )
                    + data.modulation_risk_class.get((ca, dt, risk), 0.0)
                )
...
class ReserveSharingComponent(ModelComponent):
    ...
    def build(self, context: BuildContext) -> Mapping[str, Any]:
        ...
        island_reserve_class = [(*island, rc) for island in domains.Island for rc in RESERVE_CLASSES]
        directed = [(*key, direction) for key in island_reserve_class for direction in DIRECTIONS]
        effective_domain = [
            (*key, risk)
            for key in island_reserve_class
            for risk in GEN_RISKS | MANUAL_RISKS
        ]
        block.SharedNFR = pyo.Var(
            domains.Island,
            domain=pyo.NonNegativeReals,
            bounds=lambda _b, ca, dt, island: (0.0, max(0.0, data.shared_nfr_maximum[ca, dt, island])),
        )
        ...
        block.ReserveShareEffective = pyo.Var(effective_domain, domain=pyo.NonNegativeReals)
        ...
        block.ReserveShareEffectiveCE = pyo.Var(island_reserve_class, domain=pyo.NonNegativeReals)
        block.ReserveShareEffectiveECE = pyo.Var(island_reserve_class, domain=pyo.NonNegativeReals)
        ...
        for island in domains.Island:
            ...
            block.Constraints.add(block.HVDCSent[island] <= data.big_m * block.HVDCSending[island])
            block.Constraints.add(block.HVDCSent[island] <= data.big_m * (1.0 - block.HVDCSendZero[island]))
            block.Constraints.add(sum(block.LambdaHVDCEnergy[*island, bp] for bp in ENERGY_BREAKPOINTS) == 1.0)
            block.Constraints.add(
                block.HVDCSent[island]
                == sum(data.energy_breakpoint_flow[*island, bp] * block.LambdaHVDCEnergy[*island, bp] for bp in ENERGY_BREAKPOINTS)
            )
            block.Constraints.add(
                block.HVDCSentLoss[island]
                == sum(data.energy_breakpoint_loss[*island, bp] * block.LambdaHVDCEnergy[*island, bp] for bp in ENERGY_BREAKPOINTS)
            )
        for period in case.periods:
            block.Constraints.add(sum(block.HVDCSending[island] for island in domains.Island if island[:2] == period) == 1.0)
            for reserve_class in RESERVE_CLASSES:
                block.Constraints.add(sum(block.InZone[*island, reserve_class, zone] for island in domains.Island if island[:2] == period for zone in ZONES) == 1.0)
            block.Constraints.add(
                block.ReserveSharePenalty[period]
                == 1e-5 * sum(block.SharedNFR[island] for island in domains.Island if island[:2] == period)
                + 2e-5 * sum(block.SharedReserve[key] for key in island_reserve_class if key[:2] == period)
                + 3e-5 * sum(block.ReserveShareEffectiveCE[key] + block.ReserveShareEffectiveECE[key] for key in island_reserve_class if key[:2] == period)
            )
        for key in island_reserve_class:
            ca, dt, island, reserve_class = key
            block.Constraints.add(block.HVDCSending[ca, dt, island] == sum(block.InZone[*key, zone] for zone in ZONES))
            block.Constraints.add(
                block.SharedReserve[key] <= island_reserve[key]
            )
            block.Constraints.add(
            ...
            for risk in GEN_RISKS | MANUAL_RISKS:
                ...
                block.Constraints.add(
                    ...
                    <= sum(block.ReserveShareReceived[*key, direction] * data.effective_factor.get(effective_key, 0.0) for direction in DIRECTIONS)
                )
                target = block.ReserveShareEffectiveCE[key] if risk in CE_RISKS else block.ReserveShareEffectiveECE[key]
                block.Constraints.add(target >= block.ReserveShareEffective[effective_key])
            for direction in DIRECTIONS:
                dkey = (*key, direction)
                block.Constraints.add(block.ReserveShareSent[dkey] <= block.SharedReserve[key] + (block.SharedNFR[ca, dt, island] if reserve_class == "FIR" else 0.0))
                block.Constraints.add(block.ReserveShareSent[dkey] <= max(0.0, data.hvdc_control_band[ca, dt, direction] - data.modulation_risk[ca, dt]))
                if direction == "forward":
                    block.Constraints.add(block.ReserveShareSent[dkey] + block.HVDCSent[ca, dt, island] <= max(0.0, data.hvdc_maximum[ca, dt, island] - data.modulation_risk[ca, dt]))
                    block.Constraints.add(block.ReserveShareReceived[dkey] <= data.big_m * (1.0 - block.HVDCSending[ca, dt, island]))
                    block.Constraints.add(block.HVDCReserveSent[dkey] == block.ReserveShareSent[dkey] + block.HVDCSent[ca, dt, island])
                else:
                    block.Constraints.add(block.ReserveShareSent[dkey] <= data.big_m * (1.0 - block.HVDCSending[ca, dt, island]))
                    block.Constraints.add(block.ReserveShareReceived[dkey] <= block.HVDCSending[ca, dt, island] * max(0.0, data.hvdc_control_band[ca, dt, direction] - data.modulation_risk[ca, dt]))
                    block.Constraints.add(block.ReserveShareReceived[dkey] <= block.HVDCSent[ca, dt, island] - data.monopole_minimum[ca, dt] - data.modulation_risk[ca, dt] + data.big_m * (1.0 - block.InZone[*key, "RZ"]))
                    block.Constraints.add(block.HVDCReserveSent[dkey] == block.HVDCSent[ca, dt, island] - block.ReserveShareReceived[dkey])
                block.Constraints.add(sum(block.LambdaHVDCReserve[*dkey, bp] for bp in RESERVE_BREAKPOINTS) == 1.0)
                block.Constraints.add(block.HVDCReserveSent[dkey] == sum(data.reserve_breakpoint_flow[*key[:3], bp] * block.LambdaHVDCReserve[*dkey, bp] for bp in RESERVE_BREAKPOINTS))
                block.Constraints.add(block.HVDCReserveLoss[dkey] == sum(data.reserve_breakpoint_loss[*key[:3], bp] * block.LambdaHVDCReserve[*dkey, bp] for bp in RESERVE_BREAKPOINTS))
        # Cross-island received-flow identities.
        for dkey in directed:
            ca, dt, island, reserve_class, direction = dkey
            others = [other for other in domains.Island if other[:2] == (ca, dt) and other[2] != island]
            if direction == "forward":
                block.Constraints.add(block.ReserveShareReceived[dkey] == sum(block.ReserveShareSent[*other, reserve_class, direction] - block.HVDCReserveLoss[*other, reserve_class, direction] + block.HVDCSentLoss[other] for other in others))
            else:
                block.Constraints.add(block.ReserveShareReceived[dkey] == sum(block.ReserveShareSent[*other, reserve_class, direction] for other in others) - block.HVDCReserveLoss[dkey] + block.HVDCSentLoss[ca, dt, island])
        energy_intervals = [
            (*island, left)
            for island in domains.Island
        ...
        block.LambdaHVDCEnergyInterval = pyo.Var(
            energy_intervals, domain=pyo.Binary
        )
        block.LambdaHVDCReserveInterval = pyo.Var(
            reserve_intervals, domain=pyo.Binary
        )
        ...
        if data.enforce_nmir_sos2:
            for island in domains.Island:
                ...
                for position, breakpoint in enumerate(ENERGY_BREAKPOINTS):
                    ...
                    block.PortableSOS2.add(
                        block.LambdaHVDCEnergy[*island, breakpoint]
                        <= sum(adjacent)
                    )
            for key in directed:
                block.PortableSOS2.add(
                ...
                for position, breakpoint in enumerate(RESERVE_BREAKPOINTS):
                    ...
                    block.PortableSOS2.add(
                        block.LambdaHVDCReserve[*key, breakpoint]
                        <= sum(adjacent)
                    )
        else:
...
class IslandReserveComponent(ModelComponent):
    ...
    def build(self, context: BuildContext) -> Mapping[str, Any]:
        ...
        island_class = [(*island, rc) for island in domains.Island for rc in RESERVE_CLASSES]
        block.IslandReserve = pyo.Var(island_class, domain=pyo.NonNegativeReals)
        block.IslandReserveCalculation = pyo.Constraint(
            island_class,
            rule=lambda _b, ca, dt, island, reserve_class: _b.IslandReserve[ca, dt, island, reserve_class] <= sum(
                reserve[ca, dt, offer, reserve_class, reserve_type]
...
class ReserveRequirementComponent(ModelComponent):
    ...
    supported_formulations = _SUPPORTED
    requires = frozenset(
        {"reserve_domains", "island_risk", "island_reserve"}
    )
    provides = frozenset(
    ...
    def build(self, context: BuildContext) -> Mapping[str, Any]:
        ...
        block.DeficitReserveCE = pyo.Var(
            island_class, domain=pyo.NonNegativeReals
        )
        block.DeficitReserveECE = pyo.Var(
            island_class, domain=pyo.NonNegativeReals
        )
...
class ReserveSecurityComponent(HVDCSecurityComponent):
    ...
    def build(self, context: BuildContext) -> Mapping[str, Any]:
        ...
        for name in ("MNodeSecurityConstraintLE", "MNodeSecurityConstraintGE", "MNodeSecurityConstraintEQ"):
            ...
            for key in list(component):
                ...
                extra = sum(
                    ...
                    for (ca, dt, constraint_name, offer, reserve_class, reserve_type), factor in data.market_reserve_offer_factor.items()
                    if (ca, dt, constraint_name) == tuple(key)
                )
                if extra.__class__ is not int:
                    constraint.set_value((constraint.lower, constraint.body + extra, constraint.upper))
        return artifacts
...
class ReserveEconomicsComponent(HVDCEconomicsComponent):
    ...
    def build(self, context: BuildContext) -> Mapping[str, Any]:
        ...
        block.SystemCostDefinition = pyo.Constraint(
            ...
            == sum(context.artifacts["generation_block"][key] * case.offer_price[key] for key in case.offer_blocks if key[:2] == (ca, dt))
            + sum(reserve_block[key] * data.reserve_block_price[key] for key in reserve_block if key[:2] == (ca, dt)),
        )
        ...
        block.ReserveSystemPenaltyDefinition = pyo.Constraint(
            sorted(case.periods),
            rule=lambda _b, ca, dt: _b.SystemPenaltyByPeriod[ca, dt]
            == _b.SystemPenaltyByPeriod[ca, dt] - original_penalty[ca, dt].body
            + sum(data.deficit_reserve_ce_penalty * deficit_ce[key] + data.deficit_reserve_ece_penalty * deficit_ece[key] for key in deficit_ce if key[:2] == (ca, dt)),
        )
        ...
        block.TotalScarcityCostDefinition = pyo.Constraint(
            ...
            == sum(context.artifacts["energy_scarcity_block"][key] * case.scarcity_price[key] for key in case.scarcity_blocks if key[:2] == (ca, dt))
            + sum(data.reserve_scarcity_price.get((key[0], key[1], key[2], key[-3], key[-1]), 0.0) * shortfall_block[key] for key in shortfall_block if key[:2] == (ca, dt))
            + sum(data.reserve_scarcity_price.get((key[0], key[1], key[2], key[-3], key[-1]), 0.0) * shortfall_unit_block[key] for key in shortfall_unit_block if key[:2] == (ca, dt))
            + sum(data.reserve_scarcity_price.get((key[0], key[1], key[2], key[-3], key[-1]), 0.0) * shortfall_group_block[key] for key in shortfall_group_block if key[:2] == (ca, dt)),
        )
        block.del_component(block.Objective)
        block.del_component(block.NetBenefit)
        block.NetBenefit = pyo.Expression(
            expr=block.SystemBenefit - block.SystemCost - block.SystemPenalty - block.ScarcityCost - sum(share_penalty[key] for key in case.periods) + sum(case.scarcity_limit[key] * case.scarcity_price[key] for key in case.scarcity_blocks)
        )
