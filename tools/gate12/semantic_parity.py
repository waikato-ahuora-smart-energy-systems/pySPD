...
_ZERO_SPARSE_SURFACES = frozenset(
    {"primary-physics", "rounded-published-output"}
)
_MAX_EXAMPLES = 20
...
class SemanticCaseComparator:
    ...
    def compare_surface(
        ...
        *,
        ...
        repaired_errors = self._repaired_errors(
            repaired_reference, repaired_candidate
        )
    ...
    def _unresolved_reason(
        surface: str, item: CanonicalValueDifference
    ) -> str:
        if surface in _EXACT_SURFACES:
    ...
    def _repaired_errors(
        ...
        for path in set(reference_leaves) | set(candidate_leaves):
            ...
            errors[path] = 0.0 if expected == actual else self.differ._absolute_error(
                expected, actual
            )
        return errors
...
class SemanticCaseResult:
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "surfaces": {
                name: result.to_dict()
                for name, result in sorted(self.surfaces.items())
            },
        }
