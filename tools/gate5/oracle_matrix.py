"""Compare the Pyomo AC-network matrix with the pinned vSPD Convert matrix."""
...
from pyspd.network import ac_network_formulation
...
ROW_FAMILIES = frozenset(
    {
        ...
        "ACBranchMaximumFlow",
        "ACBranchFlowDefinition",
        "LinearLoadFlow",
        "ACBranchBlockLimit",
        "ACDirectedBranchFlowDefinition",
        "ACBranchLossCalculation",
        "ACDirectedBranchLossDefinition",
...
VARIABLE_FAMILIES = frozenset(
    {
        ...
        "ACBRANCHFLOW",
        "ACNODEANGLE",
        "ACBRANCHFLOWDIRECTED",
        "ACBRANCHLOSSESDIRECTED",
        "ACBRANCHFLOWBLOCKDIRECTED",
        "ACBRANCHLOSSESBLOCKDIRECTED",
        "DEFICITBUSGENERATION",
        "SURPLUSBUSGENERATION",
        "DEFICITBRANCHSECURITYCONSTRAINT",
        "SURPLUSBRANCHSECURITYCONSTRAINT",
        "SURPLUSBRANCHFLOW",
        "DEFICITMNODECONSTRAINT",
        "SURPLUSMNODECONSTRAINT",
    }
)
PYOMO_ROWS = {
    "ACNetwork.ACNodeNetInjectionDefinition1": "ACnodeNetInjectionDefinition1",
    "ACNetwork.ACNodeNetInjectionDefinition2": "ACnodeNetInjectionDefinition2",
    "ACNetwork.ACBranchMaximumFlow": "ACBranchMaximumFlow",
    "ACNetwork.ACBranchFlowDefinition": "ACBranchFlowDefinition",
    "ACNetwork.LinearLoadFlow": "LinearLoadFlow",
    "ACNetwork.ACBranchBlockLimit": "ACBranchBlockLimit",
    "ACNetwork.ACDirectedBranchFlowDefinition": "ACDirectedBranchFlowDefinition",
    "ACNetwork.ACBranchLossCalculation": "ACBranchLossCalculation",
    "ACNetwork.ACDirectedBranchLossDefinition": "ACDirectedBranchLossDefinition",
    "NetworkSecurity.BranchSecurityConstraintLE": "BranchSecurityConstraintLE",
    "NetworkSecurity.BranchSecurityConstraintGE": "BranchSecurityConstraintGE",
    "NetworkSecurity.BranchSecurityConstraintEQ": "BranchSecurityConstraintEQ",
    "NetworkSecurity.MNodeSecurityConstraintLE": "MNodeSecurityConstraintLE",
    "NetworkSecurity.MNodeSecurityConstraintGE": "MNodeSecurityConstraintGE",
    "NetworkSecurity.MNodeSecurityConstraintEQ": "MNodeSecurityConstraintEQ",
}
PYOMO_VARIABLES = {
    "EnergyOffers.Generation": "GENERATION",
    "DemandBids.Purchase": "PURCHASE",
    "EnergyScarcity.EnergyScarcityNode": "ENERGYSCARCITYNODE",
    "ACNetwork.ACNodeNetInjection": "ACNODENETINJECTION",
    "ACNetwork.ACBranchFlow": "ACBRANCHFLOW",
    "ACNetwork.ACNodeAngle": "ACNODEANGLE",
    "ACNetwork.ACBranchFlowDirected": "ACBRANCHFLOWDIRECTED",
    "ACNetwork.ACBranchLossesDirected": "ACBRANCHLOSSESDIRECTED",
    "ACNetwork.ACBranchFlowBlockDirected": "ACBRANCHFLOWBLOCKDIRECTED",
    "ACNetwork.ACBranchLossesBlockDirected": "ACBRANCHLOSSESBLOCKDIRECTED",
    "ACNetwork.DeficitBusGeneration": "DEFICITBUSGENERATION",
    "ACNetwork.SurplusBusGeneration": "SURPLUSBUSGENERATION",
    "NetworkSecurity.DeficitBranchSecurityConstraint": "DEFICITBRANCHSECURITYCONSTRAINT",
    "NetworkSecurity.SurplusBranchSecurityConstraint": "SURPLUSBRANCHSECURITYCONSTRAINT",
    "ACNetwork.SurplusBranchFlow": "SURPLUSBRANCHFLOW",
    "NetworkSecurity.DeficitMNodeConstraint": "DEFICITMNODECONSTRAINT",
    "NetworkSecurity.SurplusMNodeConstraint": "SURPLUSMNODECONSTRAINT",
}
...
def pyomo_stage5_projection(model: pyo.ConcreteModel) -> LinearMatrixEvidence:
    fixed_values = {
        id(variable): float(pyo.value(variable))
        for variable in model.component_data_objects(pyo.Var, active=True)
        if variable.fixed
    }
    for variable in model.component_data_objects(pyo.Var, active=True):
        if id(variable) in fixed_values:
            variable.unfix()
    ...
    used_variable_ids = set(objective_by_id)
        ...
        columns.append(
            MatrixColumn(
                ...
                _zero(
                    ...
                    else _bound(variable.lb, -math.inf)
                ),
                _zero(
                    fixed_value
                    if fixed_value is not None
                    else _bound(variable.ub, math.inf)
                ),
                _zero(objective_by_id.get(id(variable), 0.0)),
                0.0,
                0.0,
                "continuous",
            )
    ...
    for constraint in model.component_data_objects(pyo.Constraint, active=True):
        ...
        rows.append(
            MatrixRow(
                name,
                _zero(_bound(constraint.lower, -math.inf) - constant),
                _zero(_bound(constraint.upper, math.inf) - constant),
                0.0,
                0.0,
            )
        )
        for variable, coefficient in zip(
            ...
            if column is not None and float(coefficient) != 0.0:
                entries.append(MatrixEntry(name, column, _zero(float(coefficient))))
    for variable in model.component_data_objects(pyo.Var, active=True):
        if id(variable) in fixed_values:
            variable.fix(fixed_values[id(variable)])
    return LinearMatrixEvidence.build(rows, columns, entries, "maximize")


def gams_stage5_projection(matrix: LinearMatrixEvidence) -> LinearMatrixEvidence:
    rows = tuple(
        MatrixRow(
            row.name,
            _zero(row.lower),
...
def compare(
    *,
    ...
    built = ModelAssembler().assemble(
        ac_network_formulation(preprocess=True),
        CaseData("vspd-v5.0.6", identifier, raw),
    )
    python = pyomo_stage5_projection(built.model)
    ...
    gams = gams_stage5_projection(dictionary.apply(scalar))
