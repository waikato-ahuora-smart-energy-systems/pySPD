...
| `G12-04` | Identical selection, order, prior-dispatch initialization, and fallback path | Historical checkpoints now require exact canonical GDX progress order; exact hash-bound application `case_ids` selection fails on missing/duplicate identities; cross-implementation state-transition comparison remains pending | Partial |
...
| `G12-09` | SCIP-MIP → fix discrete → HiGHS-RMIP portable profile passes | Representative v5/v16 pathways and independent validators pass; complete population/day execution pending | Partial |
