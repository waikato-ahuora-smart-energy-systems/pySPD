...
|---|---|---|---|
| `G12-01` | Exactly 546 unique affected identities across all 139 Gate 1 hashes | All 139 hashes screened; 434 analytic candidates remain diagnostic only. The daily-mode exact-positive selector is invalidated because three dates emitted 737 false memberships. The replacement non-daily, first-loop, actual material-transfer profile passed canonical qualification on 2022-11-06: 270/270 optimal cases and exactly four identities/six node transfers matching the independent reconstruction. Full enumeration is resuming from that hash-bound checkpoint | In progress |
| `G12-02` | All 546 cases replayed through pinned GAMS and PySPD | Incremental checkpoint feed, canonical prefix planner, durable case-streaming PySPD producer, pinned daily-mode GAMS instrumentation/export producer, twelve-surface bundle store, exact per-date parity checkpoint coordinator, and immutable path-level numeric diff artifact implemented. The live first-date candidate passed the formerly failing case `51012022111105693` and is checkpointing every successor. The GAMS producer passed its real pinned-source patch boundary but execution is queued behind the active single-node population enumeration | In progress |
